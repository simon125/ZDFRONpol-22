...
div.innerHTML = `<div style="height: 50%; width: 50%; background-color: red;"></div>`;
...
users.forEach((user, index) => {
  /**
   * Dodawanie/dobudowywanie wierszy HTMLowych na podstawie
   * elementów tablicy
   */
  usersTable =
    usersTable +
  ...
  if (index === users.length - 1) {
    /**
     * Po ostatnim wierszu owrapowywujemy (otaczamy) wiersze tabelki (<tr></tr>)
     * pozostałą wymaganą strukturą HTMLową
     */
...
const classes = ["start", "center", "end"];

const flexContainer = document.getElementById("flexContainer");

setInterval(() => {
  const className = classes[randomIntFromInterval(0, 2)];

  flexContainer.classList = `flex-container ${className}`;
}, 2000);
